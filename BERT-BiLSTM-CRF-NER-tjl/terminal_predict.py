...
ouput_model_dir = r'C:\Users\C\Documents\Tencent Files\389631699\FileRecv\semi_corpus_people_2014'
...
print('checkpoint path:{}'.format(os.path.join(ouput_model_dir, "checkpoint")))
if not os.path.exists(os.path.join(ouput_model_dir, "checkpoint")):
    raise Exception("failed to get checkpoint. going to return ")

# 加载label->id的词典
with codecs.open(os.path.join(ouput_model_dir, 'label2id.pkl'), 'rb') as rf:
    label2id = pickle.load(rf)
    id2label = {value: key for key, value in label2id.items()}

with codecs.open(os.path.join(ouput_model_dir, 'label_list.pkl'), 'rb') as rf:
...
with graph.as_default():
    ...
    saver.restore(sess, tf.train.latest_checkpoint(ouput_model_dir))
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    for (i, label) in enumerate(label_list, 1):
        label_map[label] = i
    # 保存label->index 的map
    if not os.path.exists(os.path.join(ouput_model_dir, 'label2id.pkl')):
        with codecs.open(os.path.join(ouput_model_dir, 'label2id.pkl'), 'wb') as w:
